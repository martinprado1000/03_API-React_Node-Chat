...
class UsersService {
  ...
  async login(body) {
    ...
    try {
      ...
      return {
        response: { status: 201, data: `Usuario logueado satisfactoriamente`, rol: data.rol },
        token: token,
      };
    } catch (error) {
      console.log(error);
  ...
  async recoveryPassword(body) {
    try {
      ...
      const editedPassword = await this.usersRepository.put(userFound.id, {password:randomNumberHased});
      if (editedPassword) {
        await sendMailRecoveryPassword( userFound.email, randomNumber );
        return { status: 201, data: `Se envió la nueva contraseña al correo: ${body.email}` };
      } else {
  ...
  async profile(req) {
    try {
      const user = req.user; // Al trabajar sin sessiones el req.user son los datos obtenidos del token con jwt.
  ...
  static deleteInactiveUsers = async () => {
    try {
      ...
      const deletedUsers = users.data.map(async (user) => {
        ...
        if ( user.rol == "superAdmin" || user.rol == "admin" || user.rol == "userPremium") return; 
